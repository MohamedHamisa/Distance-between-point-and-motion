# we calculating the difference between the 2 frames
#blur to reduce the noise 
#dilate and erode to reduce the noise around the edges
import cv2
import numpy as np 
...
while True:
	...
	cv2.circle(prev, (20,200), 5, (0,0,255), -1)  #from where to calculate
	for contors in contor:				
		if cv2.contourArea(contors) > 30000:
			(x,y,w,h) = cv2.boundingRect(contors)
			(x1,y1),rad = cv2.minEnclosingCircle(contors) #you need to reduce the value when the object is far and viceverse
			x1 = int(x1)
			y1 = int(y1)
